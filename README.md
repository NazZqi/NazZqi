👋 Hola, soy Ignacio

💻 Estudiante de Ingeniería Civil Informática
🚀 Enfocado en desarrollo de software, sistemas y datos

🧠 Perfil

Estudiante de Ingeniería Civil Informática con orientación a la construcción de soluciones tecnológicas reales. Me interesa el desarrollo de software eficiente, el análisis de datos y el uso de tecnologías cloud para resolver problemas de forma escalable.

Trabajo con un enfoque práctico: aprender, implementar y optimizar.

🔍 Actualmente

Desarrollo de proyectos propios (software y sistemas)

Aplicación de estructuras de datos y algoritmos en problemas reales

Exploración de servicios cloud (AWS) y plataformas de datos

Construcción de un perfil técnico sólido orientado a la industria

⚙️ Stack Tecnológico

Lenguajes
Python · JavaScript

Desarrollo
HTML · CSS

Cloud & Datos
AWS · Snowflake

Herramientas
Git · GitHub · Confluence

📌 Lo que me diferencia

Mentalidad de ingeniería: entiendo el por qué, no solo el cómo

Enfoque en soluciones prácticas, no solo teoría

Capacidad de aprendizaje rápido y adaptación tecnológica

Interés en proyectos complejos (simulación, sistemas, data)

🎯 Objetivo Profesional

Integrarme en entornos tecnológicos donde pueda aportar desarrollando software, trabajar con datos y seguir escalando mis habilidades en ingeniería.

⚡ Filosofía

"No añadir funcionalidades hasta que sean estrictamente necesarias."
